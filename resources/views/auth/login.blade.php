@extends('layouts.app_auth', ['class' => 'off-canvas-sidebar', 'activePage' => 'login', 'title' => __('Material Dashboard')])
      ...
      <form class="form" method="POST" action="{{ route('login') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          ...
          <div class="card-body">
            ...
            <div class="form-check mr-auto ml-3 mt-3">
              <label class="form-check-label">
                <input class="form-check-input" type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}> {{ __('Ricordami') }}
          ...
          <div class="card-footer justify-content-center">
            <button type="submit" class="btn btn-primary btn-link btn-lg">{{ __('ACCEDI') }}</button>
          </div>
        </div>
      </form>
      <div class="row">
        ...
        <div class="col-6 text-right">
            <a href="{{ route('register') }}" class="text-light">
                <small>{{ __('Crea nuovo account') }}</small>
            </a>
        </div>
      </div>
    </div>
